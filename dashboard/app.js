...
function detectOutputType(text, className = '') {
  ...
  // 失败类关键词
  if (/❌|✗|fail|error|失败|出错|异常|bad substitution|syntax error|not found|cannot/i.test(text)) return 'fail';
  // 结果类关键词
  if (/✅|✓|pass|success|发现|found|结果|result|报告|report|summary|评分|score|门禁|gate|共.*处|共.*个|总计/i.test(text)) return 'result';
  // 默认归为过程
  return 'process';
}

function appendLine(text, className = '', type) {
  const outputType = type || detectOutputType(text, className);
  const panelId = outputType === 'result' ? 'terminalResult' : outputType === 'fail' ? 'terminalFail' : 'terminalProcess';
  const panel = $(panelId);
  if (!panel) return;
